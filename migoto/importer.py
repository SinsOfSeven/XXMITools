...
from data.byte_buffer import MigotoFmt, NumpyBuffer
...
class ObjectImporter:
    ...
    def import_object(self, operator, context):
        object_source_folder: Path = operator.object_source_folder

        if not object_source_folder.is_dir():
            raise ConfigError(
                "object_source_folder", "Specified sources folder does not exist!"
            )
        ...
        for filename in object_source_folder.iterdir():
            if not filename.name.endswith("fmt"):
                continue
            ...
            if not ib_path.is_file():
                raise ConfigError(
                    ...
                    f"Specified folder is missing .fmt file for {fmt_path.stem}!",
                )
            ...
            obj = self.import_component(operator, context, fmt_path, ib_path, vb_path)
        ...
        if len(imported_objects) == 0:
            raise ConfigError(
                "object_source_folder",
                "Specified folder is missing .fmt files for components!",
            )
    ...
    def import_component(
        ...
        with open(fmt_path, "r") as fmt, open(ib_path, "rb") as ib, open(
            vb_path, "rb"
        ) as vb:
            ...
            try:
                ...
                raise ConfigError(
                    "object_source_folder", "Specified folder is missing Metadata.json!"
                )
            except Exception as e:
                raise ConfigError(
                    "object_source_folder", f"Failed to load Metadata.json:\n{e}"
                )
            ...
            num_shapekeys = (
                ...
                else len(getattr(obj.data.shape_keys, "key_blocks", []))
            )

            print(
                f"{fmt_path.stem} import time: {time.time()-start_time :.3f}s ({len(obj.data.vertices)} vertices, {len(obj.data.loops)} indices, {num_shapekeys} shapekeys)"
            )
